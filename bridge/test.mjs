...
try {
  ...
  // Play: with curious on, a box that appears on the table sends Spidey over to it, the diary says so, and a
  // manual command ends the errand.
  await fetch("http://127.0.0.1:18080/obstacles", { method: "POST", body: "[]" });
  robotPose = { ...robotPose, x: 50, y: 30, heading: 180 };
  const playStatus = await (await fetch("http://127.0.0.1:18080/play", { method: "POST", body: JSON.stringify({ curious: true }) })).json();
  assert.equal(playStatus.curious, true);
  await wait(1500);
  ...
  assert.equal(states.at(-1).mission.state, "navigating");
  assert.match(states.at(-1).play.diary.map((d) => d.text).join(" | "), /What is that\? Spidey goes to look at the green box\. \| A green box appeared\./);
  client.send(JSON.stringify({ type: "command", data: { id: "p2", ts: 0, robotId: "sesame-1", type: "stop" } }));
  await wait(400);
  assert.equal(states.at(-1).play.errand, null, "a manual command ends the errand");
  await fetch("http://127.0.0.1:18080/play", { method: "POST", body: JSON.stringify({ curious: false }) });
  console.log("PASS  play: a box appears, curious Spidey sets off to it, the diary records it, stop ends it");
  client.close();
