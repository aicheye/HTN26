...
// Last angle commanded to each servo, before subtrim. -1 until the first write.
// The servos have no feedback wire, so this is the commanded angle, not a measured one.
int servoAngle[8] = {-1, -1, -1, -1, -1, -1, -1, -1};

// Angles received over the WebSocket and not yet written. -1 means none.
// loop() writes them, so a message that arrives during a gait never nests servo writes.
int pendingServoAngle[8] = {-1, -1, -1, -1, -1, -1, -1, -1};
bool wsServosChanged = false;
unsigned long wsLastBroadcastMs = 0;
String wsLastCommand = "";
String wsLastFace = "";
const unsigned long WS_BROADCAST_INTERVAL_MS = 50;
...
String servosJson() {
  ...
  for (int i = 0; i < 8; i++) {
    if (i > 0) json += ",";
    json += "\"" + ServoNames[i] + "\":" + (servoAngle[i] < 0 ? String("null") : String(servoAngle[i]));
  }
  return json + "}";
}

// State message sent to WebSocket clients on connect and on every change
String stateJson() {
  return "{\"command\":\"" + jsonEscape(currentCommand) + "\",\"face\":\"" + jsonEscape(currentFaceName) +
         "\",\"servos\":" + servosJson() + "}";
}

// WebSocket messages are JSON with any of: "servos" {name: angle}, "command", "face".
void onWebSocketEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
  ...
  if (doc["servos"].is<JsonObject>()) {
    for (JsonPair kv : doc["servos"].as<JsonObject>()) {
      int idx = servoNameToIndex(String(kv.key().c_str()));
      int angle = kv.value() | -1;
      if (idx == -1 || angle < 0 || angle > 180) {
        webSocket.sendTXT(num, "{\"error\":\"Invalid servo name or angle\"}");
        return;
      }
      pendingServoAngle[idx] = angle;
    }
    currentCommand = "";  // direct servo control stops any running gait
  }

  if (doc["face"].is<const char*>()) setFace(doc["face"].as<String>());
...
void updateWebSocket() {
  ...
  if (now - wsLastBroadcastMs < WS_BROADCAST_INTERVAL_MS) return;
  if (!wsServosChanged && currentCommand == wsLastCommand && currentFaceName == wsLastFace) return;
  wsServosChanged = false;
  wsLastCommand = currentCommand;
  wsLastFace = currentFaceName;
  wsLastBroadcastMs = now;
  String json = stateJson();
...
void applyPendingServos() {
  for (int i = 0; i < 8; i++) {
    int angle = pendingServoAngle[i];
    if (angle < 0) continue;
    pendingServoAngle[i] = -1;
    if (angle != servoAngle[i]) setServoAngle(i, angle);
  }
}
...
void loop() {
  ...
  if (currentCommand == "") applyPendingServos();
...
void setServoAngle(uint8_t channel, int angle) { 
  if (channel < 8) {
    ...
    servoAngle[channel] = adjustedAngle - servoSubtrim[channel];
    wsServosChanged = true;
