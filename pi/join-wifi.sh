...
BLOCK=$(printf '\nnetwork={\n    ssid="%s"\n    key_mgmt=WPA-PSK\n    psk="%s"\n    priority=30\n}\n' "$SSID" "$PASS")
ssh -t $SSH_OPTS $PI "grep -q 'ssid=\"$SSID\"' /boot/wpa_supplicant.conf && echo 'already listed' || { printf '%s\n' '$BLOCK' | sudo tee -a /boot/wpa_supplicant.conf >/dev/null && echo added; }; echo 'rebooting the Pi'; sudo shutdown -b" 
