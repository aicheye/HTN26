...
ssh -t $SHARE $PI "cd tracker && slay -f tracker >/dev/null 2>&1; clang++ -std=c++17 -O2 tracker.cpp -o tracker \$(pkg-config --cflags --libs opencv4) -lcamapi -lsocket && ./tracker $UNIT $W $H $FOCUS" 2>&1 | tee pi/tracker-live.log
