  ...
  return (
    <>
      ...
      <Line
        ...
        lineWidth={2}
      />
      <EdgeLimit arena={arena} />
    </>
  );
}

/** The limit the robot's centre keeps to along the arena's edge, as a dashed line. */
function EdgeLimit({ arena }: { arena: Arena }) {
  const m = edgeLimit(arena);
  if (!m) return null;
  return (
    <Line
      points={[[m, m, 0.0022], [arena.width - m, m, 0.0022], [arena.width - m, arena.length - m, 0.0022], [m, arena.length - m, 0.0022], [m, m, 0.0022]]}
      color={EDGE_LIMIT} lineWidth={1.5} dashed dashSize={0.012} gapSize={0.008} transparent opacity={0.8}
    />
  );
}
