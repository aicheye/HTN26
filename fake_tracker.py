...
def main():
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            n = lambda: random.gauss(0, args.noise)
            state = {"t": int(1000 * (time.time() - t0)), "calibrated": True, "frame": [2304, 1296], "floor": [63, 63],
                     "zUp": bool(args.zup), "learned": 4, "cameraHeight": 115, "floorMarkers": 4 if self.server.unit == 3 else 3, "fps": 15,
                     "markers": [0, 1, 2, 3, 4, 5],
                     "robot": {"x": args.robot[0] + n(), "y": args.robot[1] + n(), "z": 10.5, "heading": args.robot[2] + 3 * n(), "px": [0, 0]},
                     "arm": {"x": args.arm[0] + n(), "y": args.arm[1] + n(), "z": 5.0, "heading": args.arm[2] + 3 * n(), "px": [0, 0]},
    ...
    for unit in args.units:
        s = HTTPServer(("127.0.0.1", 8000 + unit), H); s.unit = unit
